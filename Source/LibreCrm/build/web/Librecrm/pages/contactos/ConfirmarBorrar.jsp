...
<html>
    ...
    <body>
        <form action="/LibreCrm/SvLibreCrm" method="POST">
            <center>
                <div id="logo">
                    <h1>Desea eliminar este Contacto realmente?</h1>
                    <c:forEach var="fila" items="${requestScope.borrar}">
                        <input type="hidden" name="id" value="${fila.id}">
                        <h2>Nombre: ${fila.cont_first_name} ${fila.cont_last_name}</h2>
                        <h2>Titulo: ${fila.title}</h2>
                        <h2>Departamento: ${fila.department}</h2>
                        <h2>Descripcion: ${fila.description}</h2>
                    </c:forEach>
                    <br><br>
                    <input type="submit" name="boton" value="Incio" >
                    <input type="submit" name="boton" value="Borrar Contacto" >
                </div>
            </center>
        </form>
    </body>
